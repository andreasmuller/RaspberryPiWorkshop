...
void testApp::update()
{
	...
	// add particls into the space partitioning grid
	for( unsigned int i = 0; i < particles.size(); i++ )
	{
		Particle* tmpParticle = particles.at(i);
	...
	//cout << spacePartitioningIndexDistanceX << ", " << spacePartitioningIndexDistanceY << endl;
	...
	for( unsigned int particleIndex = 0; particleIndex < particles.size(); particleIndex++ )
	{
		...
		// on the maximum distance of the line connection
		...
		for( int y = startIndexY; y < endIndexY; y++ )
		{
			...
			{
				//cout << spacePartitioningGrid.at(y).at(x).size() << endl;
